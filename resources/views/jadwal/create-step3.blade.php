                ...
                <div class="form-row align-items-center mb-3">
                    ...
                    <div class="col-md-3">
                        <select class="form-control" name="hari[{{ $lomba['sub_kategori_id'] }}]">
                             @for ($i = 1; $i <= $jumlahHari; $i++) <option value="Day {{ $i }}">Day {{ $i }}</option> @endfor
                        </select>
                    </div>
                    <div class="col-md-3">
                        <input type="time" class="form-control" name="waktu_mulai[{{ $lomba['sub_kategori_id'] }}]" placeholder="Waktu Mulai">
                    </div>
                    <div class="col-md-3">
                        <input type="time" class="form-control" name="waktu_selesai[{{ $lomba['sub_kategori_id'] }}]" placeholder="Waktu Selesai">
                    </div>
                </div>
            @endforeach
